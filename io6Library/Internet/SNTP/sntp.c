...
void printf_datetime_str(time_t timestamp, int8_t timezone)
{
    time_t now;
    struct tm * ts;
    char buf[80];
    now = timestamp + timezone*3600;
...
uint16_t get_datetime_str(uint8_t * buf, time_t timestamp, int8_t timezone)
{
    time_t now;
    struct tm * ts;
    uint16_t len;
    ...
    len = sprintf((char *)buf, "%s UTC %+d", timebuf, timezone);
...
time_t get_seconds_from_ntp_server(uint8_t *buf, uint16_t idx)
{
    ...
    epoch_time = (time_t)(seconds - 2208988800);

    /*
    // for debugging
    calcdatetime(seconds);
    printf("%.4d-%.2d-%.2d %.2d:%.2d:%.2d\r\n",
            Nowdatetime.yy, Nowdatetime.mo, Nowdatetime.dd,
...
int8_t SNTP_run(time_t* timeStamp)
{
    ...
    switch(getSn_SR(NTP_SOCKET))
    {
        case SOCK_UDP:
            ...
            {
                do{
                    ret += recvfrom(NTP_SOCKET, data_buf+ret, MAX_SNTP_BUF_SIZE, (uint8_t *)&destip, &destport, 4);
